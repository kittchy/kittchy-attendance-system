...
- [ ] 「ワークスペース」を作成・編集・削除できる（例: "本業", "副業A"）
- [ ] 出勤時にどのワークスペースで打刻するか選択できる
- [ ] 同時に複数ワークスペースで勤務はできない（1つを退勤してから別に出勤）
- [ ] 履歴・グラフ・月次サマリーをワークスペースごとにフィルタできる
- [ ] Slack設定（Webhook URL・メッセージ）はワークスペースごとに独立
- [ ] ワークスペースが未作成の場合は従来通り（デフォルトワークスペース）で動作する
...
- [ ] HomePageから直接設定画面にアクセスできる（履歴ページを経由しない）
- [ ] 各ワークスペースのSlack設定状態が一目でわかる（接続済み / 未設定）
- [ ] ワークスペース管理も設定画面から行える
...
- [ ] メニューバーのトレイアイコンから出勤・退勤・休憩開始・休憩終了ができる
- [ ] トレイメニューに現在の勤務状態が表示される（例: "勤務中 - 本業"）
